      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Manage your posted tasks and track proposals</p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2 flex-wrap">
            {[['newest','Newest'],['oldest','Oldest'],['budget_high','Highest Budget']].map(([val, label]) => (
              <Button key={val} variant={sortBy === val ? 'default' : 'outline'} size="sm" onClick={() => setSortBy(val)}>
                {label}
              </Button>
            ))}
          </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setStatusFilter('all')}
            className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-all',
              statusFilter === 'all' ? 'bg-primary text-primary-foreground border-primary' : 'bg-transparent text-muted-foreground border-border'
            )}
          >All</button>
            ...
            return (
              <button key={status} onClick={() => setStatusFilter(statusFilter === status ? 'all' : status)}
                className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-all flex items-center gap-1.5',
                  statusFilter === status ? config.color : 'bg-transparent text-muted-foreground border-border'
                )}
              >
        ...
        <div className={cn(viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4' : 'space-y-3')}>
          {tasks.map((task) => {
            ...
            return (
              <Card key={task.id}
                className={cn('border-border hover:border-primary/30 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 cursor-pointer group', viewMode === 'list' && 'flex flex-row')}
                  ...
                  <div className={cn('flex-1', viewMode === 'list' && 'min-w-0')}>
                    ...
                    <div className="flex items-start justify-between mb-3 gap-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {task.urgency === 'high' && (
                          <Badge className="bg-destructive/15 text-destructive border-destructive/30 text-xs animate-pulse">Urgent</Badge>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button className="text-muted-foreground hover:text-destructive transition-colors shrink-0 p-1" onClick={(e) => handleDeleteTask(task.id, e)}>
                          <Trash2 className="w-4 h-4" />
                        </button>
                        <button className="text-muted-foreground hover:text-foreground transition-colors shrink-0 p-1" onClick={(e) => e.stopPropagation()}>
        ...
        {tasks.length === 0 && !isLoading && (
          <div className="text-center py-16">
            ...
            {(searchQuery || selectedCategory !== 'All' || statusFilter !== 'all') ? (
              ...
              <Link to="/client/tasks/new"><Button>Post Your First Task</Button></Link>
            )}
          </div>
        )}
        ...
        {pagination.lastPage > 1 && (
          <div className="flex items-center justify-center gap-2 pt-4">
            ...
            <span className="text-sm text-muted-foreground">Page {currentPage} of {pagination.lastPage}</span>
